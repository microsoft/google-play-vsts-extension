...
if [ $1 == "createtest" ] ; then
  ...
  tfx extension create --manifest-globs vsts-extension-google-play.json --override '{ "public": "false", "name": "Google Play-Dev", "id": "google-play-dev", "publisher": "ms-vsclient"}' --share-with raujaiswal  --token $PUBLISH_ACCESSTOKEN
fi

if [ $1 == "publishtest" ] ; then
  echo "Creating and publishing test VSIX..."
  tfx extension publish --manifest-globs vsts-extension-google-play.json --override '{ "public": "false", "name": "Google Play-Dev", "id": "google-play-dev", "publisher": "ms-vsclient"}' --share-with raujaiswal --token $PUBLISH_ACCESSTOKEN
fi